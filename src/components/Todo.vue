      ...
      <transition name="fade-right">
        <div v-if="local.deadline">
          <div class="block date">
            ...
            <div @click.stop="setDeadline()" class="button">
              <Icon image="cross" size="tiny"></Icon>
            </div>
            <transition name="fade-in">
...
<style scoped>
  ...
  .todo { box-shadow: 0 5px 15px 0px rgba(0,0,0,.1); background: white; border-radius: 5px; padding: 20px; transition: padding .5s, max-height .5s, box-shadow .25s, margin .5s; }
  .compact { user-select: none; max-height: 20px; padding: 10px 20px; overflow: hidden; transition: padding .5s, max-height .5s, box-shadow .25s, margin .5s; }
  .full { margin: 10px 0; animation: overflow .5s forwards; }
